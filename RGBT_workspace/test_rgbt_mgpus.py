...
def genConfig(seq_path, set_type, seq_name):
    if set_type == 'RGBT234':
        ############################################  have to refine #############################################
        ...
        ############################################  have to refine #############################################
        if seq_name == 'Otcbvs':
            RGB_img_list = sorted([seq_path + '/v/' + p for p in os.listdir(seq_path + '/v') if os.path.splitext(p)[1] == '.bmp'])
            T_img_list = sorted([seq_path + '/i/' + p for p in os.listdir(seq_path + '/i') if os.path.splitext(p)[1] == '.bmp'])
        else:
            RGB_img_list = sorted(
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, debug=0, script_name='prompt'):
    ...
    if 'LasHeR' in dataset_name:
        save_path = f'./RGBT_workspace/results/{dataset_name}/' + save_name + '/' + seq_txt + '.txt'
    else:
        save_path = f'./RGBT_workspace/results/{dataset_name}/' + save_name + '/' + script_name+'_'+seq_txt + '.txt'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='GTOT', help='Name of dataset (GTOT,RGBT234,LasHeR,VTUAVST,VTUAVLT).')
    ...
    if dataset_name == 'GTOT':
        seq_home = '/home/cscv/Documents/lsl/dataset/GTOT'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/home/cscv/Documents/lsl/dataset/RGB-T234/'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/media/cscv/d00985a0-c3e6-4ffa-9546-88c861db5ce3/02_Dataset/LasHeR/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/media/cscv/d00985a0-c3e6-4ffa-9546-88c861db5ce3/02_Dataset/VTUAV'
