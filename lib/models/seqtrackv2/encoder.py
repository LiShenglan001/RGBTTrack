...
class EncoderBase(nn.Module):
    ...
    def forward(self, template_list, search_list, text_src, seq, z_anno, x_anno, current_epoch=None):
        if self.training:
            xs, volume_loss = self.body(template_list, search_list, text_src, seq, z_anno, x_anno, current_epoch=current_epoch)
            return xs, volume_loss
        else:
            xs = self.body(template_list, search_list, text_src, seq, z_anno, x_anno, current_epoch=current_epoch)
            return xs
